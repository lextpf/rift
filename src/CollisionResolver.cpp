...
float CollisionResolver::CalculateFollowAlpha(float deltaTime, float settleTime, float epsilon)
{
...
bool CollisionResolver::CollidesWithNPC(const glm::vec2& bottomCenterPos,
                                        const std::vector<glm::vec2>* npcPositions) const
{
    ...
    float playerMinX = bottomCenterPos.x - PlayerCharacter::HALF_HITBOX_WIDTH + EPS;
    float playerMaxX = bottomCenterPos.x + PlayerCharacter::HALF_HITBOX_WIDTH - EPS;
    float playerMaxY = bottomCenterPos.y - EPS;
    float playerMinY = bottomCenterPos.y - PlayerCharacter::HITBOX_HEIGHT + EPS;
...
bool CollisionResolver::CollidesWithTilesCenter(const glm::vec2& bottomCenterPos,
                                                const Tilemap* tilemap) const
{
    ...
    glm::vec2 centerPos(bottomCenterPos.x,
                        bottomCenterPos.y - PlayerCharacter::HITBOX_HEIGHT * 0.5f);
...
bool CollisionResolver::CollidesWithTilesStrict(const glm::vec2& bottomCenterPos,
                                                ...
                                                bool diagonalInput) const
{
    ...
    const float TILE_W = static_cast<float>(tilemap->GetTileWidth());
    const float TILE_H = static_cast<float>(tilemap->GetTileHeight());
    constexpr float HALF_W = PlayerCharacter::HITBOX_WIDTH * 0.5f;
    constexpr float BOX_H = PlayerCharacter::HITBOX_HEIGHT;
    constexpr float EPS = 0.05f;
...
bool CollisionResolver::ShouldSkipDiagonalTile(const TileOverlapContext& ctx) const
{
...
bool CollisionResolver::ShouldTolerateWallPenetration(const TileOverlapContext& ctx) const
{
...
bool CollisionResolver::ShouldAllowCornerCut(const TileOverlapContext& ctx,
                                             const Tilemap* tilemap,
...
bool CollisionResolver::CollidesAt(const glm::vec2& bottomCenterPos,
                                   const Tilemap* tilemap,
                                   ...
                                   bool diagonalInput) const
{
    ...
    if (sprintMode)
    {
        ...
        if (diagonalInput && !centerHit && tilemap)
        {
            ...
            glm::vec2 center(bottomCenterPos.x,
                             bottomCenterPos.y - PlayerCharacter::HITBOX_HEIGHT * 0.5f);
            int baseTileX = static_cast<int>(std::floor(center.x / tileW));
            int baseTileY =
                static_cast<int>(std::floor((center.y - GameCharacter::COLLISION_EPS) / tileH));
...
bool CollisionResolver::IsCornerPenetration(const glm::vec2& bottomCenterPos,
                                            const Tilemap* tilemap) const
{
    if (!tilemap)
        return false;
    ...
    const float TILE_W = static_cast<float>(tilemap->GetTileWidth());
    const float TILE_H = static_cast<float>(tilemap->GetTileHeight());
    constexpr float EPS = GameCharacter::COLLISION_EPS;

    float minX = bottomCenterPos.x - PlayerCharacter::HALF_HITBOX_WIDTH + EPS;
    float maxX = bottomCenterPos.x + PlayerCharacter::HALF_HITBOX_WIDTH - EPS;
    float maxY = bottomCenterPos.y - EPS;
    float minY = bottomCenterPos.y - PlayerCharacter::HITBOX_HEIGHT + EPS;
...
glm::vec2 CollisionResolver::ComputeSprintCornerEject(const Tilemap* tilemap,
                                                      const std::vector<glm::vec2>* npcPositions,
                                                      glm::vec2 normalizedDir) const
{
    if (!tilemap)
        return glm::vec2(0.0f);
    bool strictlyColliding = CollidesWithTilesStrict(m_Player->m_Position, tilemap, 0, 0, false);
    if (!strictlyColliding && !IsCornerPenetration(m_Player->m_Position, tilemap))
        return glm::vec2(0.0f);
    ...
    for (int dy = -MAX_STEP; dy <= MAX_STEP; ++dy)
    {
        for (int dx = -MAX_STEP; dx <= MAX_STEP; ++dx)
        {
            ...
            glm::vec2 candidatePos = m_Player->m_Position + offset;
                                                     ...
                                                     int /*moveDirY*/)
{
    ...
    glm::vec2 step = testPos - m_Player->m_Position;
    ...
    bool cornerEmptyRight = false;
    {
        ...
        float hitboxCenterY = testPos.y - PlayerCharacter::HITBOX_HEIGHT * 0.5f;
        ...
        if (horizontalPrimary)
        {
            ...
            forwardTileX =
                (step.x < 0)
                    ? static_cast<int>(
                          std::floor((testPos.x - PlayerCharacter::HALF_HITBOX_WIDTH) / TILE_W))
                    : static_cast<int>(
                          std::floor((testPos.x + PlayerCharacter::HALF_HITBOX_WIDTH) / TILE_W));

            int hitboxTopTileY =
                static_cast<int>(std::floor((testPos.y - PlayerCharacter::HITBOX_HEIGHT) / TILE_H));
        ...
        else
        {
            ...
            forwardTileY = (step.y < 0)
                               ? static_cast<int>(std::floor(
                                     (testPos.y - PlayerCharacter::HITBOX_HEIGHT) / TILE_H))
                               : static_cast<int>(std::floor(testPos.y / TILE_H));

            int hitboxLeftTileX = static_cast<int>(
                std::floor((testPos.x - PlayerCharacter::HALF_HITBOX_WIDTH) / TILE_W));
            int hitboxRightTileX = static_cast<int>(
                std::floor((testPos.x + PlayerCharacter::HALF_HITBOX_WIDTH - 0.01f) / TILE_W));
        ...
        if (!foundAnyCorner)
        {
            ...
            if (m_Player->m_SlideCommitTimer <= 0.0f)
                m_Player->m_SlideHysteresisDir = glm::vec2(0.0f);
            return glm::vec2(0.0f);
        }
    ...
    auto evalDir = [&](const glm::vec2& dir, float maxProbe) -> Eval
    {
        ...
        for (int magInt = 1; magInt <= static_cast<int>(maxProbe); ++magInt)
        {
            ...
            if (hardTileBlocked(m_Player->m_Position + offset, sdx, sdy))
                continue;

            e.canSlideOnly = true;

            // slide + forward must be safe
            if (!hardTileBlocked(m_Player->m_Position + offset + forward, fdx, fdy))
            {
                e.canForward = true;
                e.bestMag = mag;
                break;
    ...
    float playerOffset = 0.0f;  // Negative = toward dNeg, Positive = toward dPos
    {
        ...
        {
            wallTileY = (step.y < 0) ? static_cast<int>(std::floor(
                                           (testPos.y - PlayerCharacter::HITBOX_HEIGHT) / TILE_H))
                                     : static_cast<int>(std::floor(testPos.y / TILE_H));
    ...
    auto preferredFirst = [&]() -> std::array<glm::vec2, 2>
    {
        if (horizontalPrimary)
        {
            ...
            if (m_Player->m_SlideHysteresisDir.y < 0.0f)
                return {dNeg, dPos};
            if (m_Player->m_SlideHysteresisDir.y > 0.0f)
                return {dPos, dNeg};
            if (m_Player->m_LastInputY < 0)
                return {dNeg, dPos};
            if (m_Player->m_LastInputY > 0)
                return {dPos, dNeg};
        ...
        else
        {
            ...
            // Nearly centered - use hysteresis/last input
            if (m_Player->m_SlideHysteresisDir.x < 0.0f)
                return {dNeg, dPos};
            if (m_Player->m_SlideHysteresisDir.x > 0.0f)
                return {dPos, dNeg};
            if (m_Player->m_LastInputX < 0)
                return {dNeg, dPos};
            if (m_Player->m_LastInputX > 0)
                return {dPos, dNeg};
    ...
    if (glm::length(chosen) > 0.001f)
    {
        ...
        else
        {
            ...
            blockTileY = (step.y < 0) ? static_cast<int>(std::floor(
                                            (testPos.y - PlayerCharacter::HITBOX_HEIGHT) / TILE_H))
                                      : static_cast<int>(std::floor(testPos.y / TILE_H));
            int hitboxLeftTileX = static_cast<int>(
                std::floor((testPos.x - PlayerCharacter::HALF_HITBOX_WIDTH) / TILE_W));
    ...
    if (glm::length(chosen) > 0.001f)
    {
        if (glm::length(m_Player->m_SlideHysteresisDir) < 0.001f ||
            glm::dot(chosen, m_Player->m_SlideHysteresisDir) <
                0.5f)  // Direction changed significantly
        {
            m_Player->m_SlideCommitTimer = 0.12f;  // Commit to this direction for 120ms
        }
        m_Player->m_SlideHysteresisDir = chosen;
    }

    return chosen;
}

glm::vec2 CollisionResolver::FindClosestSafeTileCenter(
    const Tilemap* tilemap, const std::vector<glm::vec2>* npcPositions) const
{
    ...
    int baseTileX = static_cast<int>(std::floor(m_Player->m_Position.x / TILE_W));
    int baseTileY = static_cast<int>(std::floor((m_Player->m_Position.y - TILE_H * 0.5f) / TILE_H));

    float bestDist2 = std::numeric_limits<float>::infinity();
    glm::vec2 bestCenter = m_Player->m_Position;
    ...
    for (int dy = -2; dy <= 2; ++dy)
    {
        for (int dx = -2; dx <= 2; ++dx)
        {
            ...
            {
                float dist2 = glm::dot(bottomCenterPos - m_Player->m_Position,
                                       bottomCenterPos - m_Player->m_Position);
                if (dist2 < bestDist2)
                {
                    bestDist2 = dist2;
...
glm::vec2 CollisionResolver::TrySlideMovement(glm::vec2 desiredMovement,
                                              ...
                                              bool diagonalInput)
{
    ...
    glm::vec2 testPos = m_Player->m_Position + desiredMovement;
    ...
    {
        m_Player->m_SlideHysteresisDir = glm::vec2(0.0f);
        m_Player->m_SlideCommitTimer = 0.0f;
        return glm::vec2(0.0f);
    }
    ...
    auto attemptDir = [&](glm::vec2 dir) -> glm::vec2
    {
        ...
        for (int slideAmountInt = 1; slideAmountInt <= 16; ++slideAmountInt)
        {
            ...
            glm::vec2 testSlideForward = m_Player->m_Position + slideOffset + forwardProbe;
                            ...
                            diagonalInput))
            {
                ...
                if (CollidesAt(m_Player->m_Position + clampedOffset,
                               tilemap,
                               npcPositions,
                               slideSprintMode,
                               (int)dir.x,
                ...
                for (int i = 0; i < 8; ++i)
                {
                    ...
                    glm::vec2 tryPos = m_Player->m_Position + clampedOffset + forwardMove * mid;
                ...
                if (!CollidesAt(m_Player->m_Position + blended,
                                tilemap,
                                npcPositions,
                                slideSprintMode,
                                moveDx,
            ...
            glm::vec2 testSlideOnly = m_Player->m_Position + slideOffset;
...
glm::vec2 CollisionResolver::ApplyLaneSnapping(glm::vec2 desiredMovement,
                                               glm::vec2 normalizedDir,
                                               float deltaTime,
                                               const Tilemap* tilemap,
                                               const std::vector<glm::vec2>* npcPositions,
                                               ...
                                               int moveDy)
{
    ...
    glm::vec2 bottomCenterPos =
        m_Player->GetCurrentTileCenter(static_cast<float>(tilemap->GetTileWidth()));
    glm::vec2 offsetToCenter = bottomCenterPos - m_Player->m_Position;
    ...
    if (movingHorizontal)
    {
        ...
        glm::vec2 testPos = m_Player->m_Position + glm::vec2(desiredMovement.x, correction);
        ...
        else
        {
            ...
            glm::vec2 testPerpOnly = m_Player->m_Position + glm::vec2(0.0f, correction);
    ...
    else
    {
        ...
        glm::vec2 testPos = m_Player->m_Position + glm::vec2(correction, desiredMovement.y);
        ...
        else
        {
            ...
            glm::vec2 testPerpOnly = m_Player->m_Position + glm::vec2(correction, 0.0f);
...
void CollisionResolver::HandleIdleSnap(float deltaTime,
                                       const Tilemap* tilemap,
                                       const std::vector<glm::vec2>* npcPositions)
{
    float tileSize = tilemap ? static_cast<float>(tilemap->GetTileWidth()) : 16.0f;
    glm::vec2 targetCenter = m_Player->GetCurrentTileCenter(tileSize);
    float distanceToCenter = glm::length(targetCenter - m_Player->m_Position);

    // === Stuck Detection: teleport to safety if inside collision ===
    if (tilemap && CollidesWithTilesStrict(m_Player->m_Position, tilemap, 0, 0, false))
    {
        m_Player->m_Position = FindClosestSafeTileCenter(tilemap, npcPositions);
        targetCenter = m_Player->GetCurrentTileCenter(tileSize);
        distanceToCenter = glm::length(targetCenter - m_Player->m_Position);
    }
    else if (tilemap)
    {
        // Track last known safe position for recovery
        m_Player->m_LastSafeTileCenter = m_Player->GetCurrentTileCenter(tileSize);
    }
    ...
    if (distanceToCenter > 0.5f)
    {
        ...
        if (targetCenter != m_Player->m_SnapTargetPos || m_Player->m_SnapProgress >= 1.0f)
        {
            m_Player->m_SnapStartPos = m_Player->m_Position;
            m_Player->m_SnapTargetPos = targetCenter;
            m_Player->m_SnapProgress = 0.0f;
        }
        ...
        m_Player->m_SnapProgress += deltaTime / snapDuration;
        m_Player->m_SnapProgress = std::min(m_Player->m_SnapProgress, 1.0f);

        // Apply smoothstep to total progress for ease-in/ease-out: t*t*(3.0 - 2.0*t)
        float t = m_Player->m_SnapProgress;
        float smoothT = t * t * (3.0f - 2.0f * t);

        // Calculate target position using smoothed progress
        glm::vec2 desiredPos = m_Player->m_SnapStartPos +
                               (m_Player->m_SnapTargetPos - m_Player->m_SnapStartPos) * smoothT;
        glm::vec2 snapMovement = desiredPos - m_Player->m_Position;
